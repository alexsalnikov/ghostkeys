...
import { expandTmux, tmuxGroupPurpose, tmuxGroups, tmuxLessons, tmuxWhy, type TmuxLesson } from './tmuxLessons';
...
export function TmuxApp({onPathChange}:{onPathChange(path:LearningPath):void}){
  ...
  return <div className="app-shell tmux-app">
    ...
    <main className="main-content"><header className="topbar"><span>tmux · {progress.overview?'Your terminal, kept together':lesson.group}</span><span>{learned} / {tmuxLessons.length} learned</span></header>
      ...
      {progress.overview?<div className="workspace tmux-overview"><div className="eyebrow">A SEPARATE PATH · SAME FRIENDLY GHOSTS</div><h1>Keep your terminal haunt alive.</h1><p className="intro">tmux organizes terminal work at three levels: a session contains full-screen windows, and each window can be divided into visible panes.</p>
        <div className="tmux-overview-grid"><section className="panel"><h2>Two ways to multitask</h2><p><strong>Switch windows</strong> when each task needs the whole terminal, like switching tabs. Ctrl+b then n or p changes which window fills the screen.</p><p><strong>Split panes</strong> when programs should stay visible together, such as code beside logs. Ctrl+b then % or &quot; divides the current window.</p></section>
          <section className="panel"><h2>Session → window → pane</h2><div className="tmux-concept"><strong>Session: ghost · one project</strong><div>Window 0: code · full-screen tab<div>Pane: Vim · Pane: tests</div></div><div>Window 1: services · full-screen tab<div>Pane: server · Pane: logs</div></div></div><p>Only one window fills the terminal at a time. Every pane in that window remains visible.</p></section></div>
        <section className="panel tmux-quickstart"><h2>Choose the right level</h2><div><code>Ctrl+b → c</code><span>Create another full-screen window</span></div><div><code>Ctrl+b → n / p</code><span>Switch full-screen windows</span></div><div><code>Ctrl+b → % / &quot;</code><span>Split the current window into panes</span></div><div><code>Ctrl+b → arrows / o</code><span>Focus another visible pane</span></div></section>
      ...
      </div>:<div className="workspace training-workspace tmux-workspace"><div className="lesson-topline"><div className="eyebrow">TMUX · {lesson.group.toUpperCase()}</div><span className="lesson-count">Lesson {index+1} of {tmuxLessons.length}</span></div><div className="title-row"><div><h1>{lesson.title}.</h1><p className="intro">{lesson.description}</p><p className="tmux-why"><strong>Why this helps:</strong> {tmuxWhy[lesson.id]}</p></div></div>
